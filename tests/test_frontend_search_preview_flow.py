...
def test_profile_routes_to_unified_search_setup_without_an_extra_mission_page() -> None:
    ...
    assert "Answer the required clarifications before starting the search." in intent_form
    assert "Review detected conflicts" in intent_form
    assert "conflictsAccepted" in intent_form
