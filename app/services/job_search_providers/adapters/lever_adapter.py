...
DEFAULT_LEVER_LISTING_URLS = [
    "https://jobs.lever.co/figma",
]


class LeverAdapter(BaseHTTPJobSiteAdapter):
    name = "lever"
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            listing_urls=listing_urls or DEFAULT_LEVER_LISTING_URLS,
            fetcher=fetcher,
    ...
    def _parse_listing_html(self, html: str, listing_url: str) -> list[RawJobCandidate]:
        ...
        for href, body in anchor_pattern.findall(html):
            title = extract_first(
                [
                    r'data-qa="posting-name"[^>]*>(.*?)</',
                    r'class="[^"]*posting-name[^"]*"[^>]*>(.*?)</',
                    r"<h\d[^>]*>(.*?)</h\d>",
                ],
                body,
            )
            location = extract_first(
                [
                    r'class="[^"]*posting-category[^"]*location[^"]*"[^>]*>(.*?)</',
                    r'class="[^"]*sort-by-location[^"]*"[^>]*>(.*?)</',
                    r'data-qa="posting-location"[^>]*>(.*?)</',
                ],
                body,
            )
