...
DEFAULT_CUHKSZ_LIST_URL = "https://career.cuhk.edu.cn/job/search/d_category/102"
...
class CUHKSZParser(JobSiteParser):
    ...
    def parse_list(self, html: str, base_url: str) -> list[RawJobListItem]:
        ...
        for node in list_nodes:
            ...
            time_node = node.select_one(".sousuo_list_time")
    ...
    def parse_detail(self, html: str, item: RawJobListItem) -> RawJobDetail:
        extracted_text, extraction_warnings = extract_cuhksz_detail_text(html)
        fallback_text = _build_fallback_text(item)
        ...
        return RawJobDetail(
            ...
            extraction_method="cuhksz_html",
        )


def extract_cuhksz_detail_text(detail_html: str) -> tuple[str, list[str]]:
    warnings: list[str] = []
...
def _build_fallback_text(item: RawJobListItem) -> str:
    lines = [
        f"Title: {item.title}",
