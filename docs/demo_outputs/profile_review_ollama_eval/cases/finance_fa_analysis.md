# Finance / FA analysis resume

## Resume summary
- case_id: finance_fa_analysis
- target_roles: Business Analyst, Investment Analyst

## Baseline parsed result
- baseline_confidence_label: strong
- baseline_quality_warnings: -
- baseline_missing_info_questions: -
- parsed_skills: Excel, PowerPoint, Wind, CRM, market research, competitor analysis, industry research, competitive landscape, deal memo, meeting notes, peer mapping, market size, FA
- parsed_project_count: 2
- parsed_work_experience_count: 3
- parsed_education_count: 1
- parsed_highlights: Excel, PowerPoint, Wind, CRM, market research, competitor analysis, Used Wind and Qichacha for industry research, peer mapping, and deal memo support., Prepared CRM updates, meeting notes, and competitor analysis summaries., New Energy Sector Research, Analyzed market size, business models, and competitive landscape for listed companies.

## Profile draft summary
- draft: {"skill_count": 13, "project_count": 2, "work_experience_count": 3, "education_count": 1, "highlight_count": 5, "target_roles": ["Business Analyst", "Investment Analyst"]}

## LLM enrichment result
- enrichment_enabled: True
- enrichment_suggestion_count: 0
- enrichment_llm_success_count: 0
- enrichment_fallback_count: 6
- enrichment_discarded_suggestion_count: 0
- enrichment_quality_warnings: LLM profile enrichment unavailable; using deterministic baseline only

## Simulated user decisions
- accepted_suggestion_count: 0
- edited_suggestion_count: 0
- rejected_suggestion_count: 0

## Confirmed profile result
- confirmed_confidence_label: strong
- confirmed_skill_count: 13
- confirmed_project_count: 2
- confirmed_work_experience_count: 3
- confirmed_remaining_warnings: -

## Save payload readiness
- save_payload_ready: True

## Quality verdict
- quality_verdict: strong

## Reviewer notes
- fallback_count=6
