# Realistic unstructured business / FA resume

## Resume summary
- case_id: realistic_business_resume_unstructured
- target_roles: Business Analyst, Investment Analyst, FA Intern

## Baseline parsed result
- baseline_confidence_label: strong
- baseline_quality_warnings: -
- baseline_missing_info_questions: -
- parsed_skills: FA, Wind, 企查查, Excel, PowerPoint, CRM
- parsed_project_count: 1
- parsed_work_experience_count: 3
- parsed_education_count: 0
- parsed_highlights: 工作内容包括维护 CRM 项目跟进表，记录客户需求，写会议纪要，跟进投资人反馈，识别高意向潜在客户。也参与过新材料行业研究，整理市场空间、商业模式、产业链上下游和竞品情况。, 目标方向是 Business Analyst、Investment Analyst、FA Intern 或者行业研究相关实习，希望能在深圳、香港或杭州做商业分析、投资分析、行业研究相关工作。

## Profile draft summary
- draft: {"skill_count": 6, "project_count": 1, "work_experience_count": 3, "education_count": 0, "highlight_count": 2, "target_roles": ["Business Analyst", "Investment Analyst", "FA Intern"]}

## LLM enrichment result
- enrichment_enabled: True
- enrichment_suggestion_count: 0
- enrichment_llm_success_count: 4
- enrichment_fallback_count: 0
- enrichment_discarded_suggestion_count: 6
- enrichment_quality_warnings: discarded ungrounded profile enrichment suggestion

## Simulated user decisions
- accepted_suggestion_count: 0
- edited_suggestion_count: 0
- rejected_suggestion_count: 0

## Confirmed profile result
- confirmed_confidence_label: strong
- confirmed_skill_count: 6
- confirmed_project_count: 1
- confirmed_work_experience_count: 3
- confirmed_remaining_warnings: -

## Save payload readiness
- save_payload_ready: True

## Quality verdict
- quality_verdict: acceptable

## Reviewer notes
- missing expected skills: industry research, competitor analysis, meeting notes
- discarded 6 unsupported enrichment suggestions
